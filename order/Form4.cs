...
namespace TurnTableDemo
{
    public partial class Form4 : Form
    {
        ...
        {
            List<(string Name, decimal Price)> selectedItems = new List<(string Name, decimal Price)>();

            foreach (var item in listBox1.SelectedItems)
            {
                string itemText = item.ToString();
                var parts = itemText.Split(new[] { " " }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length >= 2)
                {
                    string name = string.Join(" ", parts.Take(parts.Length - 1)); // 除去最后一个元素（价格）的所有部分
                    if (decimal.TryParse(parts.Last().TrimEnd('元'), out decimal price)) // 假设价格是字符串的最后一部分，并且以“元”结尾
                    {
                        selectedItems.Add((name, price));
                    }
                }
            }

            Form5 form5 = new Form5(selectedItems);
            form5.ShowDialog();
        }
